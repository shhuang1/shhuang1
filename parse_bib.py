    ...
    for entry in bib_database.entries:
        ...
        if os.path.isfile(filenm):
            ...
            with open(filenm, 'w', encoding="utf8") as the_file:
                ...
                # Exclude PDF links for now
                #if 'link' in entry:
                #    the_file.write('url_pdf = "'+supetrim(entry['link'])+'"\n')
                url_custom_dict = dict()
                if 'doi' in entry:
                    url_custom_dict['DOI'] = 'http://dx.doi.org/'+supetrim(entry['doi'])
                if 'pmid' in entry:
                    url_custom_dict['PubMed'] = 'http://www.ncbi.nlm.nih.gov/pubmed/'+supetrim(entry['pmid'])
                if 'url_journal' in entry:
                    url_custom_dict['View at journal'] = supetrim(entry['url_journal'])
                the_file.write('url_custom = [ '+
                               ',\n'.join('{name="%s",url="%s"}'%(k,v) for k,v in url_custom_dict.items()) +
                               '\n ] \n')
